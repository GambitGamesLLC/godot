...
## 2026-05-19 audit addendum — Tonemap specialization seam after commit `1c2d852d`

The fresh QA artifact at `/home/derrick/.openclaw/workspace/.temp/gdgs-stage-repro-2026-05-19/official-tonemap-specialization-seam-vulkan-sourcebuild-20260519-164911/` supports the claimed specialization classification, but only as a sharper bucket read — not as full isolation of the poisoned boundary. The new compare block does advance the seam *inside* the current Tonemap-owned poisoned-boundary candidate: `specialization_delta` is no longer just a raw hash mismatch, it now resolves concretely to Tonemap/base having no specialization constants while the immediate `Command Graph (L88) (Draw)` neighbor carries exactly one `int` specialization constant (`id=0`, `value=0`). The saved log shows `specialization_delta={relation="different_specialization_recipe",hash_changed=true,id_hash_changed=true,value_hash_changed=true,count_changed=true,type_mask_changed=true,...}` with `base={count=0,...,preview=[]}` and `other={count=1,type_mask="0x2",int_count=1,min_id=0,max_id=0,preview=[{id=0,type="int",bits="0x0",value=0}]}`.

That is real progress, but it still does **not** collapse the poisoned boundary to specialization constants alone. The same immediate neighboring-pass compare remains broad at `recipe_delta.changed_components=["vertex_input_recipe", "blend_recipe", "specialization_constants", "pipeline_layout"]`, while the older narrowing lanes remain reduced instead of reopening: `bind_owned_attachment={class="direct_pipeline_bind_exhausted",narrower_bind_owned_seam="none",...}` still exhausts the serial-8 bind-owned lane, `setup_pair_contract={contract_class="pair_contract_clean",next_seam_candidate="pipeline_owned_state",...}` still keeps the `8 -> 9` handshake clean, and `tonemap_to_l88_transition={backend_gap_commands=0,...,first_meaningful_expansion="l88_label"}` still shows no hidden backend gap between Tonemap and `L88`.

Auditor conclusion: the exact next tightest backend-owned seam is now the **Tonemap -> L88 pipeline-layout delta in the immediate neighboring-pass pipeline compare**. Vertex input is structurally explained, blend is structurally explained, and specialization constants are now structurally explained (`none` vs one trivial `int` constant). The remaining least-explained bucket is pipeline layout, and the earlier clean Tonemap bind/setup contract only de-risks Tonemap’s own local `8 -> 9` handshake — it does not explain the neighboring-packet layout divergence between Tonemap and `L88`. That makes pipeline layout the tightest remaining backend-owned seam to split next.
